...
if ( true === ( $force_show_all_schedule_definitions = empty( $private_schedule_definitions ) ) ) {
	...
	if ( class_exists( 'WP_Hook') ) {
		...
		if ( isset( $wp_filter['cron_schedules']) ) {
			$cron_schedules_filters = $wp_filter['cron_schedules'];
			...
			foreach ( $filter_functions as $function ) {
				$func = '';
				$fn = '';
				if ( is_array( $func = $function['function'] ) ) {
					// The filter callable should be defined as a object->method
					if ( is_object( $obj = $func[0] ) ) {
						$robj = new ReflectionObject( $obj );
						$fn = $robj->getFileName();
					}
				} elseif ( is_callable( $func = $function['function'] ) ) {
				...
				if ( 0 === strpos( $fn, get_stylesheet_directory() ) ) {
					...
					$filter_functions_owners[ $owner ] = array( 'owner' => $owner, 'function' => $func, 'type' => 'theme', 'suspect' => false );
				} elseif ( 0 === strpos( $fn, str_replace( plugin_basename( __FILE__ ), '', __FILE__ ) ) ) {
					$owner = strtolower( array_shift( explode( '/', plugin_basename( $fn ) ) ) );
					$filter_functions_owners[ $owner ] = array( 'owner' => $owner, 'function' => $func, 'type' => 'plugin', 'suspect' => false );
				} else {
					...
					$owner = strtolower( array_shift( explode( '/', plugin_basename( $fn ) ) ) );
					$filter_functions_owners[ $owner ] = array( 'owner' => $owner, 'function' => $func, 'type' => 'muplugin', 'suspect' => false );				
				}
			...
			foreach ( $filtered_filter_functions_owners as $owner => &$params ) {		
				$a = call_user_func( $params['function'], array( 'itbub_test_tag' => array() ) );
...
//$show_all_schedule_definitions = false;
